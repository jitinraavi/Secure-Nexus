...
import { isInfraType, buildInfraMesh } from "../lib/infra";
...
export function Canvas3D({
  ...
  useEffect(() => {
    ...
    if (infraGroupRef.current) {
      scene.remove(infraGroupRef.current);
      disposeGroup(infraGroupRef.current);
      infraGroupRef.current = null;
    }
    if (!isInfraType(projectType) || !design.infra) return;
    const { widthMm: W, depthMm: D } = design.room;
    const mesh = buildInfraMesh(projectType, design.infra, W * MM, D * MM);
    scene.add(mesh);
    infraGroupRef.current = mesh;
  }, [projectType, design.infra, design.room.widthMm, design.room.depthMm]);
